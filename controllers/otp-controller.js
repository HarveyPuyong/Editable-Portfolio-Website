  ...
  try {
    ...
    if (existingOtp) {
      ...
      if (diff < COOLDOWN_SECONDS) {
        return res.status(429).json({ message: `Please wait ${Math.ceil(COOLDOWN_SECONDS - diff)} seconds before requesting another OTP` });
      }
    ...
    await otpEmailer(user.email, "Your OTP Code", `Your OTP is ${otpCode}. It will expire in 15 minutes.`);

    res.status(200).json({ message: "OTP sent successfully", cooldown: COOLDOWN_SECONDS });
  } catch (error) {
    console.error(error);
...
const verifyOtp = async (req, res) => {
  try {
    const { otp } = req.body; // validated by middleware
